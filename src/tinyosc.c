...
/** The OSC protocol specifies big-endian data transfer, and this must be
 * enforced. The following endianness functions are completely independent on
 * the host platform, which maximizes portability, as illustrated within Rob
 * Pike's article 'The data order fallacy'
...
static uint32_t tosc_vwrite(char *buffer, const int len, const char *address,
...
uint32_t tosc_writeNextMessage(tosc_bundle *b, const char *address,
  ...
  if (b->bundleLen >= b->bufLen)
    return 0;
  const uint32_t i = tosc_vwrite(b->marker + 4, b->bufLen - b->bundleLen - 4,
                                 address, format, ap);
  va_end(ap);
